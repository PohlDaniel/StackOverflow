...
struct RenderV2P {
	...
	centroid float3 bitangent : TEXCOORD7;
};
...
float3 SSSSTransmittance(float translucency,
        float sssWidth,
	...
	float3 profile = weights[0] * exp(dd / variances[0]) +
...
float3 SSSSTransmittance2( float translucency, 
						   float sssWidth, 
						   float3 worldPosition, 
						   float3 worldNormal, 
						   float3 light, 
...
float4 RenderPS(RenderV2P input,
	...
	*/
	...
	//float4 albedo = float4(0.0, 0.45, 0.75, 1.0);;
	float3 specularAO = float3(0.6, 0.2, 0.9);
