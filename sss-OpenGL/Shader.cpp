...
void Shader::bindAttributes(Mesh *a_mesh, GLuint texture1, GLuint texture2){
...
void Shader::loadSampler(std::string location, int sampler){

	glUseProgram(m_program);
	glUniform1i(glGetUniformLocation(m_program, location.c_str()), sampler);
	glUseProgram(0);
}

//OpenGL specifies matrices as column-major to get row-major just transpose it

void Shader::loadMatrix(std::string location, const Matrix4f matrix){

	glUniformMatrix4fv(glGetUniformLocation(m_program, location.c_str()), 1, true, &matrix[0][0]);

}

void Shader::loadMatrix(std::string location, const Matrix4f matrix, int index) {
	glUniformMatrix4fv(glGetUniformLocation(m_program, (location + "[" + std::to_string(index) + "]").c_str()), 1,true, &matrix[0][0]);

}

void Shader::loadVector(std::string location, Vector2f vector) {

	glUniform2fv(glGetUniformLocation(m_program, location.c_str()), 1, &vector[0]);
}


void Shader::loadVector(std::string location,  Vector2f vector, int index) {

	glUniform2fv(glGetUniformLocation(m_program, (location + "[" + std::to_string(index) + "]").c_str()), 1, &vector[0]);
}

void Shader::loadVector(std::string location,  Vector3f vector){

	glUniform3fv(glGetUniformLocation(m_program, location.c_str()), 1, &vector[0]);
}

void Shader::loadVector(std::string _location, Vector3f vector, int index) {

	glUniform3fv(glGetUniformLocation(m_program, (_location + "[" + std::to_string(index) + "]").c_str()), 1, &vector[0]);
}

void Shader::loadFloat2(std::string location, float value[2]){

	glUniform1fv(glGetUniformLocation(m_program, location.c_str()), 2, value);
}

void Shader::loadFloat(std::string location, float value){

	glUniform1f(glGetUniformLocation(m_program, location.c_str()), value);
}


void Shader::loadBool(std::string location, bool value){
...
///////////////////////////////Subsurface////////////////////////////
SubsurfaceShader::SubsurfaceShader(std::string vertex, std::string fragment) : Shader(vertex, fragment){
	loadSampler("u_colorTex", 0);
	loadSampler("u_depthTex", 1);
	loadSampler("u_strengthTex", 2);
}

SubsurfaceShader::~SubsurfaceShader(){

}

void SubsurfaceShader::bindAttributes(GLuint colorTexture, GLuint depthTexture, GLuint strengthTexture) {
	...
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, strengthTexture);

	glEnableVertexAttribArray(texCoordID);
	glVertexAttribPointer(texCoordID, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(3 * sizeof(float)));
}

void SubsurfaceShader::unbindAttributes() {
...
///////////////////////////////Specular////////////////////////////
SpecularShader::SpecularShader(std::string vertex, std::string fragment) : Shader(vertex, fragment) {
	loadSampler("u_specularsTex", 0);
}
...
void SpecularShader::bindAttributes(GLuint specularsTexture) {

	glEnableVertexAttribArray(positionID);
	glVertexAttribPointer(positionID, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0);

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, specularsTexture);

	glEnableVertexAttribArray(texCoordID);
	glVertexAttribPointer(texCoordID, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(3 * sizeof(float)));
}

void SpecularShader::unbindAttributes() {
